...
use crate::{mq::MediaQueue, routes};

/// Serves axum backend server
pub async fn serve() {
    let mq = MediaQueue::new();

    // build our application with a route
    // bind routes
    let router = get_router(mq).await;
...
pub async fn get_router(mq: MediaQueue) -> Router {
    // Prepare swagger
    let swagger = SwaggerUi::new("/v1/swagger").url("/v1/api-docs/openapi.json", routes::ApiDoc::openapi());

    routes::bind_routes(mq.clone(), Router::<MediaQueue>::new())
        .merge(swagger)
